...
const AddArticle = (props) => {
  ...
  useEffect(() => {
    const Edit = location.search !== '' ? true : false
    if (Edit) {
  ...
  const getDetailFromRedux = (isArticle) => {
    const detailObj = isArticle ? props.articles.filter((item) => item._id === id)[0] : props.drafts.filter((item) => item._id === id)[0]
    // 解构赋值
  ...
  const addToDB = (dbName) => {
    const page = dbName === 'articles' ? '/articles' : '/drafts'
    const messages = dbName === 'articles' ? '文章发布成功！' : '草稿保存成功！'
    const icon = dbName === 'articles' ? <CarryOutOutlined style={{ color: 'blue' }} /> : <FileDoneOutlined style={{ color: 'blue' }} />
    db.collection(dbName)
      .add({
        title,
        titleEng,
  ...
  const turnDraft = () => {
    ...
    // 判断英文标题是否存在
    const sameEngInDrafts = props.drafts.filter((item) => item.titleEng === titleEng).filter((item) => item._id !== id)
    // 如果分类存在，直接返回
  ...
  const turnArticle = () => {
    ...
    // 判断英文标题是否存在
    const sameEngInArticles = props.articles.filter((item) => item.titleEng === titleEng).filter((item) => item._id !== id)
    // 如果分类存在，直接返回
  ...
  return (
    <div className='ml-100 mt-10'>
      ...
      <div className='justify-around pb-10 bold fs-16'>
        ...
        <div>
          文章分类：
          <Select showSearch allowClear style={{ width: '260px' }} value={selectClasses} onChange={(value) => setSelectClasses(value ? value : '')}>
